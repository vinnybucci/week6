...
In the folder with this README, there's a `campground.sql` SQL script that drops and recreates the `npcampground` database. Remember to run this script before you work on the exercises below to ensure that your database is in a good state.
...
There's a `BaseDAOTests` class that all the tests inherit from. This class sets up the test data for the integration tests to run against, and then restore the original data. The requirements below explain what data is expected for the tests that you write.
...
If you use the start date as today + 3, and the end date as today + 5, the test data returns two sites in the test park.
